...
module.exports = class Registrator
{
  ...
  {
    if (! (extraContactUriParams instanceof Object))
    {
      extraContactUriParams = {};
    }

    const contact = Grammar.parse(this._contact, 'Contact')[0].parsed;
    
    contact.uri.clearParams();
    
    for (const param_key in extraContactUriParams) {
	const param_value = extraContactUriParams[param_key];
