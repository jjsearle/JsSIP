...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _handleSessionTimersInIncomingRequest(request, responseExtraHeaders)
  {
    ...
    // Emit a new event so session timers can be handled externally	  
    this.emit('runsessiontimer');
    // Remove original built in method
  ...
  // Called externally to trigger a session timer update as refresher
  execSessionTimerAsRefresher()
  {
    if (this._status === C.STATUS_TERMINATED) { return; }
    
    debug('execSessionTimerAsRefresher() | sending session refresh request');
  ...
  // Called externally to end session if session timer update not received
  execSessionTimerAsNotRefresher()
  {
    if (this._status === C.STATUS_TERMINATED) { return; }
    
    debugerror('execSessionTimerAsNotRefresher() | timer expired, terminating the session');
...
//  NOTE: Original JsSIP function for handling session timers,
//        this does not work well with react native as timers do not work 
//        when in the background / proximity sensor is activated
//
//  _runSessionTimer()
//  {
